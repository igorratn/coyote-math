...
- **Ambiguity-stump corollary (Apr 20, 2026)**: when the MCQ has two defensible options and the model picks the one the annotator didn't, that's **evidence of ambiguity, not stump**. Model answering A while annotator marks B on a prompt where both A and B are ≥15%-probability reads of the same pixels = G2 fail (Type 12 / Type 1), NOT a valid stump. Rule: if you can articulate a reasonable reading under which the model's answer is correct, stump fails. Incident: Marketing_Dashboard_59 A? — coral top band with MCQ "C. Orange / D. Red"; model C, annotator D; R1 correctly called Type 12 thumbs-down.
