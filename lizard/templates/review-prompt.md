...
- G2 Single Verifiable Answer: one deterministic answer, no subjectivity. **Multi-step prompts (codified 2026-04-28 from `Report_Dashboard_Metrics_Dashboard_74` A3 NV return):** any prompt that chains operations (e.g., "estimate the ratio? Add that to the count of bars...") MUST end with an explicit closing instruction like `What is the result? Answer with a single integer (e.g., 40).` A bare format hint in parens at the end is insufficient when the prompt body has multiple imperative clauses. Missing closing = G2 fail (no clean single-output spec).
- G3 Self-Contained: answerable from the image alone. **Acceptable: World Knowledge** (general public facts a typical person would have — Mother's Day date, value of pi, $ = US dollars, red octagon = Stop, classical elements). **REJECT: specialized / domain knowledge** (ML/stats jargon like "kernel PCA", "RBF bandwidth", "linearly separable"; chemistry expertise; medical / legal / niche-domain expertise; specific data like AAPL stock price). If interpreting the prompt requires specialized knowledge — even just to parse the question wording, not just to answer it — the prompt fails G3. **Thumbs-down with G3 fail in flags.** (Codified 2026-04-29 after Plot_Dimensionality_reduction_graphs_156: opus auto-resolved 👍 on 3 prompts using ML jargon "transformed feature representations", "RBF kernel bandwidth γ", "convex / linearly separable" — should have been 👎 G3.)
- G4 Independence: no reference to other annotations
- G5 No Giveaways: doesn't over-specify to the point of triviality. **Format-example leakage (codified 2026-04-28 from `Report_Dashboard_Metrics_Dashboard_74` A1 NV return):** if the format example uses a template that pre-structures the answer (e.g., `(e.g., all chart types [number] times)` for a "which appears most" question — telegraphs that all categories tie at the same count), the format itself leaks the answer. Treat as G5 fail. Reviewer must read the format example as if it were part of the prompt body, not a neutral hint.
...
- Type 12: Indistinguishable colors in image — also **chart-type classification ambiguity (codified 2026-04-28 from `Report_Dashboard_Metrics_Dashboard_74` A1/A2 NV return):** prompts that depend on classifying visually-similar chart types (line vs bar-with-many-narrow-bars, donut vs pie, stacked vs grouped bar) must pixel-verify both legend/axis cues AND rendered shapes at zoom. If the chart could plausibly be read as either type, the prompt's enumeration premise fails Type 12.
...
- **Specialized-knowledge gate (codified 2026-04-29):** before accepting a 👍, check whether interpreting the prompt requires **specialized / domain knowledge** (ML/stats, chemistry, medicine, niche-domain). Indicators: prompt vocabulary an educated non-expert would not parse (e.g., "kernel PCA", "convex hull", "p-value", "stoichiometry", "amortized complexity"). If yes → **thumbs-down, flag as G3 fail**, regardless of whether the rewrite answer is numerically correct. World Knowledge (general public facts) is acceptable; specialized knowledge is not. See §1 G3.
- **Stump rule:** model answer must differ from rewrite answer. If they match → thumbs-down (annotator failed to stump the model). **V6 explicit (codified 2026-04-28 per V6 playbook §Step 4):** "correct answer **independent of format**" is too easy and invalid. A model answer that is numerically/semantically equal but format-mismatched (e.g. model `0.7` vs rewrite `0.7%`) is still considered correct → still thumbs-down. Format strictness was a V5 rule; V6 inverts it.
- **Near-miss is not a stump (codified 2026-04-27 from NV return on Plot_Statistical_significance_graphs_60 A1):** if the model answer differs from the rewrite by only a deterministic rounding / arithmetic precision artifact (e.g. last-decimal-digit drift, model returns `2.32666` vs correct `2.32715`; relative diff < 0.1% on a numeric answer; single-digit edit on an integer answer), the prompt does NOT show a meaningful model failure even when the annotator's answer is technically correct. Rate **thumbs-down** unless the prompt explicitly tests precision (e.g. "round to 4 decimals"). The prefilter pre-flags these as `G3_NEAR_MISS` (soft) — confirm by checking the math chain yourself.
...
- `**Edits Made:**` — list ONLY edits relative to the **annotator's** rewrite, not relative to the model's answer. If your `Final Rewrite Answer` equals the annotator's rewrite, do NOT write "Corrected the final answer" — there is no edit to record (you computed the same value the annotator submitted). The model's answer is irrelevant to this field; "the model was wrong" goes in the Two-Part Check stump analysis, not here. Valid edits to list: skill tag corrections (with FROM/TO values, e.g. `Dropped Spatial Reasoning`), prompt rewordings (with before/after), qtype flips (e.g. `MCQ → Short answer question`), and answer corrections that actually change the annotator's value (with FROM/TO, e.g. `Corrected final answer from 60 to 24`). Use `None` when nothing changes vs the annotator's submission.
