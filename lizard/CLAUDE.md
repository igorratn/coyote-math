...
**Skill-audit gate at Job 2 (codified 2026-05-06).** Job 2 merger now runs the skill-audit heuristics on every annot bound for `auto-resolved` (👍-close path). If the post-edit final skill set is missing any heuristic-required skill (Enumeration on "count"/"how many"/"identify all"; Math on "sum"/"mean"/"average"/"difference"/"divide"/"multiply"/"subtract"; TCG on "chart"/"graph"/"table"/"gauge"/"axis"/"bar"/"panel"), the annot is demoted to `pending-igor` so Igor walks it at 3a and adds the missing skill via Igor Verdict's `skills_check`. Reason: the original 2026-04-30 audit only fired *before presenting to Igor* — auto-resolved annots bypassed it entirely. Implemented in `job2-merge.mjs:auditPromptSkills()`.

**Skill-audit gate at Job 5 — pre-Save (codified 2026-05-06).** `sa-push-devbrowser.sh` calls `node scripts/skill-audit-sanity.mjs <STEM>` before navigating to SA. If the script exits non-zero (any annot fails audit), the push refuses and the SA editor never loads — defense-in-depth in case the Job 2 gate didn't catch it (e.g. payload was hand-written, or new heuristic was added later). Bypass with `SKIP_SKILL_AUDIT=1` when Igor has explicitly accepted the mismatches.
