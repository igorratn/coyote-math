...
- **Timeout recovery: skip stuck task, rerun fire-stem — never hand-roll form fill (codified 2026-05-08).** When `fire-shadow-devbrowser.sh` times out mid-form (any phase), the in-progress HAI task remains live and HAI's "Start task" will RESUME it instead of allocating a fresh one. The script's Phase 1 then fills `TASK_ID_FIELD` (stem.json) into whatever textarea it finds first — which is the prompt textarea if reminders are already past — corrupting the form. Recovery is ALWAYS: open dev-browser tab `hai-lizard-job4` → click Skip → confirm "Yes, skip task" → wait for fresh task allocation → rerun `STEM=<S> bash scripts/fire-stem.sh` (idempotent, skips already-fired annots). NEVER hand-roll dev-browser scripts to inspect/refill/upload manually on the stuck task — React handlers may be dead (clicks register but `filechooser` never fires, even with the correct `button[aria-label="Upload assets"]` selector), and you'll burn round-trips reproducing logic the production script already has. Reason: 2026-05-08 V5 batch incident — Soil_155 A1 timed out at step-3→step-4 (303-char prompt, possible HAI silent no-op); agent spent ~8 round-trips hand-rolling refill+upload before Igor flagged "use the script's file picker selector!"; reload + skip + rerun fire-stem completed the same work in 2 round-trips.
