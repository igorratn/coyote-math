...
**Cycle-3 intake guard (codified 2026-05-05):** queue-intake refuses any stem with both `payloads/done/<S>.yaml` AND `payloads/done/<S>.cycle1.yaml` present (status `skip-already-done-cycle3`). That state means cycle 2 already finalized — re-queue would be cycle 3, not supported. Interactive picker flags such rows with a 🛑 marker before selection. Same guard at top of `run-job1.mjs` (defense-in-depth in case the queue file was hand-written). SA queue tab can show finalized rows pre-refresh (status update lags until Igor sets the task-level QC dropdown in SA UI), so the guard prevents accidentally re-processing them. Cycle-1-only done (`done/<S>.yaml` without `.cycle1.yaml`) is the legitimate cycle-2 return path and is allowed. Reason: 2026-05-05 in-flight-classification incident — `Financial_Futures_149` and `Transportation_00001` both had `.yaml` + `.cycle1.yaml` in done/ from earlier in the session; the agent misclassified them as "new candidates" because it only checked the `.cycle1.yaml` archive, not the current-cycle payload.
...
7. **Cycle-2 prompt-diff stamp (codified 2026-05-05):** for each cycle-2 returnee, compare its prompt text against the cycle-1 prompt in `tasks/<S>.cycle1.md`. Stamp the skeleton per-annot with `prompt_changed_cycle2: true | false`. Comparison: literal byte-equality after stripping leading/trailing whitespace and collapsing internal whitespace runs to single spaces. Cycle-1 annots get no such field (not applicable). Drives Job 4 delete-shadow gate per Angie Z. ruling 2026-04-21 (see `wiki/slack-rulings.md` 2026-05-05 entry): unchanged prompt = no save effort warranted = no shadow on delete.
8. Consistency check: post-filter `N_ANNOTATIONS ≥ 1`, every `prompt_len ≥ 50`, every annotator answer non-empty. Any failure → exit 1.
9. Write `tasks/skeleton/<S>.md` with: Task Info (task_id, SA_TASK_FILENAME, image path, date, review cycle) + per-annotation sections (skills, qtype, model answer, annotator answer, prompt, QC feedback, `prompt_changed_cycle2` field on cycle-2 annots, empty Two-Part Check / Edits / Feedback placeholders).
...
### Steps
1. **Claude-as-opus (codified 2026-05-06):** Before invoking `run-job2.mjs`, the orchestrator (this conversation, also `claude-opus-4-7`) generates `/tmp/lizard/<S>/opus-review.md` directly. Read `tasks/skeleton/<S>.md` for prompt + per-annot metadata, read `screenshots/<S>.<ext>` for the image, then write the file using the standard template format (`## Annotation N` blocks per `templates/review-template.md`: Rating, Final Rewrite Answer, Flags, Two-Part Check, Edits Made, Feedback). Same model as the `claude` CLI subprocess would have spawned, no API call, full conversation context (CLAUDE.md rules, codified incidents, prior-cycle awareness). `run-job2.mjs` detects the pre-existing file and skips the opus subprocess automatically (codified at `runReviewer()` idempotency check). Other reviewers (gpt/gemini/grok) still fire as subprocesses (different APIs, different judgments).
2. **Run review + merge:** `STEM=<S> node scripts/run-job2.mjs`. Fire order: opus → gpt → gemini → grok. Opus is skipped (file already exists from step 1); gpt/gemini/grok fire sequentially. Per-annot filtering: each reviewer sees only annots still pending after upstream auto-resolves. First-👍-wins early-stop. Override fire order with `REVIEWERS=a,b,c` env (rare; defaults are policy).
3. Inside `run-job2.mjs`:
...
**Skill-edit paren-depth guard (codified 2026-05-06 — Data_Analytics_4 A1 incident):** `parse-edits-made.mjs` now ignores skill tokens that appear at deeper paren-depth than the verb that scopes them. Reason: opus's edit `"Dropped Logical Reasoning (computing a mean of an enumerated set is Math Reasoning, not conditional/branching inference)"` was parsed as dropping BOTH Logical Reasoning AND Math Reasoning, because the prior parser put any token within a verb's sentence-scope into that verb's bucket. The reviewer's narrative explicitly cited Math Reasoning as the JUSTIFICATION for keeping it, but parser dropped it anyway. Result in SA: A1 had Math Reasoning incorrectly removed; sanity script `skill-audit-sanity.mjs` flagged it post-batch ("missing Math Reasoning"). Fix: tokens INSIDE parens are treated as justifications/citations, not action targets — only same-paren-depth-as-verb tokens count. Top-level comma lists still work (`"Drop A, B"` → both dropped); paren-bound mentions don't (`"Drop A (because B is the right tag)"` → only A dropped).

**Skill-audit gate at Job 5 — pre-Save (codified 2026-05-06).** `sa-push-devbrowser.sh` calls `node scripts/skill-audit-sanity.mjs <STEM>` before navigating to SA. If the script exits non-zero (any annot fails audit), the push refuses and the SA editor never loads. Bypass with `SKIP_SKILL_AUDIT=1` when Igor has explicitly accepted the mismatches.

**End-of-batch sanity check (codified 2026-05-06).** `skill-audit-sanity.mjs` (no arg) scans all done + shadow_applied payloads, runs the same heuristics, prints all mismatches without refusing. Useful for surveying historical drift / pre-batch sanity. Same heuristics as the Job 2 + Job 5 gates above.
...
5. **CLI appends the Igor Verdict block** under the annotation (immediately after the reviewer bodies, before any closing `---`):
   ...
   No manual save-attempt knob — the cycle-2 delete-shadow gate (`prompt_changed_cycle2`) is auto-derived at Job 1 from prompt-text diff vs `tasks/<S>.cycle1.md`. Igor never has to flag save effort manually.
...
annotations:
  - n: <int>                        # annotation index (1..N)
    sa:
      ...
      prompt_changed_cycle2: false  # cycle-2 only (omit on cycle 1). Auto-derived at Job 1 from prompt-text diff vs tasks/<S>.cycle1.md. Gates whether Job 4 fires a shadow on action: delete. true → fire shadow (annotator rewrote, reviewer evaluated, still failed); false → skip shadow (unchanged prompt = no save effort warranted).
...
- `sa.prompt_changed_cycle2` ← auto-derived at Job 1 (prompt-text diff vs `tasks/<S>.cycle1.md`), carried through Job 2 skeleton and Job 3b. Cycle-1 annots: field absent. Only consumed by Job 4 when `sa.action == delete`; otherwise ignored.
...
2. **Delete-shadow gate (codified 2026-05-05 — Angie Z. ruling 2026-04-21):** if `sa.action == delete` AND `sa.prompt_changed_cycle2 != true` → SKIP this annot's shadow entirely. Record `{n, shadow_skipped: true, reason: "cycle2_prompt_unchanged"}` in sidecar `payloads/shadow_applied/<S>.shadows.yaml` for audit; no HAI form fires. Reason: HAI no longer pays shadows for quick unsalvageable cycle-2 deletes. Operationalized as "annotator changed the prompt" (auto-detected at Job 1, deterministic) — unchanged prompt → no save effort warranted → no shadow. Applies uniformly to Auto Verdict and Igor Verdict deletes.
3. **Determine HAI form content** (only reached if gate passed):
   - `action: delete` AND `prompt_changed_cycle2: true` → prompt = `Deleted Annotation`, answer = `Deleted Annotation`, HAI rating = Reject
...
4. **Fill via `scripts/fill-hai-shadow.js` blobs** (~6 round-trips):
...
### Hard rules
- **1 shadow per annot reviewed, EXCEPT cycle-2 deletes where annotator left the prompt unchanged** (codified 2026-05-05). Per Angie Z. ruling 2026-04-21: HAI no longer pays shadows on quick unsalvageable cycle-2 deletes. The save-effort signal is auto-derived from prompt-text diff at Job 1 (`prompt_changed_cycle2`) — no manual knob. Fire shadow when `sa.action == delete AND sa.prompt_changed_cycle2 == true`. All non-delete annots (approve / QC_Return) still fire 1 shadow each. Cycle-1 annots: field absent, no gate (cycle-1 deletes don't exist; cycle-1 + 👎 = QC_Return).
...
- **QC feedback check before role click — PUSH-THROUGH policy (codified 2026-04-30, refined 2026-05-05, refined again 2026-05-06).** After LLM validation, before clicking "Reviewing": extract QC feedback text (above "Are you annotating or reviewing this task?"). Print it. If it contains anything other than "looks good"/"may continue"/"no issues" AND `verdict_source == auto` — record as `hai_llm_eval=warning` with the QC text as `hai_llm_comment`, stamp the warning to `tasks/<S>.md` under the annotation's `#### HAI QC Warning` block, AND CONTINUE THE FIRE (do NOT halt). Igor-verdict annots are gate-immune. Reason: ~50% of QC warnings are pure prompt-phrasing nits ("Otherwise...the answer is correct") that don't change the verdict; halting on every one is wasteful theater (Data_Analytics_102 incident 2026-05-06: A1 + A3 both halted on prompt-grammar nits, both Igor-approved within 1 min each). Push through, let Job 5 resolution gate surface the warnings as a single batch review at the end. Same logic as post-submit gate — Igor's adjudication is the ground truth, not HAI's inline LLM nudge.
- **Live-resolution auto-mark (codified 2026-05-06).** When fire-stem.sh records a warning shadow AND Igor approves the warning in conversation during the active batch session (e.g. "approve resolved" / "approve override"), immediately run `STEM=<S> node scripts/mark-resolved.mjs` after recording — don't queue the gate clearance for Igor to repeat at Job 5. Igor's conversation-time approval IS the audit record; forcing him to re-confirm via mark-resolved at end-of-batch is redundant work for the same decision. If Igor says "reject" / "QC_Return" / "flip to N", run `run-reclaim.mjs` (flip + /reclaim) instead. Only DON'T auto-resolve when Igor explicitly defers ("I'll look at this later").
...
- **Use dev-browser, not chrome-devtools-mcp, for Job 4 form fill (codified 2026-05-05).** Invoke via `STEM=<S> ANNOT_N=<n> RATING=<Approve|Reject> IMG_PATH=<path> TASK_ID_FIELD=<S>.json PROMPT_FILE=<path> ANSWER=<text> [VERDICT_SOURCE=auto|igor] bash scripts/fire-shadow-devbrowser.sh`. Image transits the QuickJS sandbox as base64 (via `~/.dev-browser/tmp/`) → buffer payload to Playwright `filechooser.setFiles({name, mimeType, buffer})` (sandbox blocks `setInputFiles(path)` because `platform.fs()` is stubbed). Mirrors the 2026-05-05 dev-browser-for-Job-5 codification. Empirical (this batch, 6 shadows): ~30–40s/shadow vs chrome-mcp baseline ~60–120s; LLM validation accounts for ~20s of the wall-clock and is unavoidable. The sandbox emits an emscripten cleanup assertion at script end on some runs — benign, all output captured before it fires.
- **HAI cross-allocation freshness gate uses LABEL detection, not empty-textarea (codified 2026-05-09 — Home_buying_checklist_179 corruption).** Earlier gate checked only "is the textarea pre-filled?" — fails when the cross-allocated task is on Step 3 with the prompt slot still empty (prior fire bailed before filling). The gate now reads `document.body.innerText` for the visible step label: Step 1 has "Copy the ID of the task that you are working on from SuperAnnotate", Step 3 has "Copy over the Annotator Prompt from SuperAnnotate", Step 4 has "Copy over the Rewrite Answer from SuperAnnotate". If `onStep1` is false, abort cleanly with `FRESHNESS_FAIL=not_on_step1`. Reason: 2026-05-09 incident — fire-stem hit cross-allocation, Step 1 empty-check passed (textarea was empty), script wrote `Infographic_Home_buying_checklist_179.json` into the Step 3 prompt slot, corrupting that task's prompt. Recovery (per Igor 2026-05-09 override): may overwrite the corrupted slot and push through to completion, then `/reclaim` later if needed; do not abandon a task we just corrupted.

- **HAI cross-allocation guard: verify task is fresh BEFORE filling (codified 2026-05-08, revised diagnosis).** "Start task" can return ANOTHER reviewer's mid-flow task instead of allocating a fresh one for the caller — HAI's allocation pool occasionally serves an abandoned/in-progress task to whoever clicks Start task next. Phase 0 captures a UUID via the `/run` URL pattern, but the page state may already be past Step 1 with another stem's content prefilled. Phase 1 then fills `TASK_ID_FIELD` (caller's stem.json) into whatever textarea it finds first — which lands in the prompt slot at Step 3 if reminders are already past — corrupting another reviewer's form (and on a read-only/locked task, achieving nothing). Phase 0 MUST verify the task is fresh before proceeding: timer at 00:00:00, no `Edit this step` buttons present, no Step-1/2/3 content already filled, OR explicit reminders/Continue button visible. If any of those checks fail, abort the fire (do NOT click Skip — that may dispose of someone else's work; just exit cleanly and the operator decides). Recovery: rerun `STEM=<S> bash scripts/fire-stem.sh` (idempotent, skips already-fired annots; HAI may serve a different task on retry). NEVER hand-roll dev-browser scripts to inspect/refill/upload manually — React handlers on someone-else's task may be dead, the form may be read-only, and the caller burns round-trips on a task they were never supposed to touch. Reason: 2026-05-08 V5 batch incident — fire-stem.sh's Phase 0 for Soil_155 A1 captured UUID `1755d343`, which was actually Training_Dashboard_162 A4 from another reviewer (mid-Reviewing/Reject, fully filled, never recorded in our pipeline). The agent's Phase 1 wrote `Plot_Soil_analysis_charts_155.json` into Training_162's prompt textarea before the readback gate caught it. The actual Soil_155 A1 fire later succeeded under fresh UUID `19f3523f` after we abandoned the cross-allocated task. Initial diagnosis (own-prior-timeout cascade) was wrong; HAI's allocation pool is the real source.

### DOM mechanics
HAI form-fill helpers in `scripts/fire-shadow-devbrowser.sh` (dev-browser pipeline; `scripts/fill-hai-shadow.js` is the legacy chrome-mcp version, kept for reference). Selectors + native-setter patterns in inline comments + `wiki/hai-selectors.md`. Post-submit LLM verdict capture: TODO — DOM probe details to be added when warning modal selector is identified.
...
4. **Pre-save audit (mandatory, codified 2026-05-01; expanded 2026-05-06):** for every annot, readback ALL written fields against the payload before clicking Save:
   - **Annotator Question (prompt) readback (codified 2026-05-06 — V6 batch incident):** for every annot, read the textarea labeled "Annotator Question" and verify it equals `hai.prompt` from the payload. ALWAYS check, even on 👎. Reason: in the V6 batch, `sa-apply.js` walked up from QC container at depth 8 (`fb-droppable-main`) and took `tas[0]` as Rewrite Answer — but `tas[0]` at that level is actually the *Annotator Question* (prompt). 12 prompts across 8 stems were overwritten with answer values; 1 (CS_30 A1) unrecoverable because the SA task moved out of queue before restore could fire. Prompt-readback would have caught this before Save.
   ...
   - **Find write-targets by LABEL, not walk-up + position (codified 2026-05-06).** sa-apply.js's `findLabeledTextarea(doc, label, annotIndex)` anchors on the panel's title element ("Rewrite Answer", "Annotator Question", "Feedback for QC", etc.). The prior walk-up + `tas[0]` heuristic relied on a false invariant ("top textarea in walk-up parent = Rewrite Answer") that breaks at depth ≥8 where the parent spans the entire annot panel (prompt + model_answer + rewrite + explanation + …). Never use position-in-array to identify a write target — always anchor on the section header.
   - **Dedupe by title element when multiple textareas share a panel (codified 2026-05-06 — Dashboard batch incident).** SA's V6 multi-annot Dashboard tasks render TWO textareas per "Annotator Question" panel: the visible prompt textarea + a hidden JSON metadata textarea (containing `[{"type":"answer","latency":...,"request_status":"PASS",...}]`). The original `findLabeledTextarea` returned ALL textareas under matching titles → 5 annots × 2 textareas = 10 matches → `matches[idx]` returned wrong textareas with off-by-one shift (matches[1] = A1's metadata, not A2's prompt). Fix: dedupe by the `titleEl` itself — first textarea per unique title element wins (the visible/editable one is always first in DOM order). Audit caught it before Save; without prompt readback, 5 stems' worth of prompts would have been overwritten with JSON.
   - **--finalize is conditional on SAVED=true (codified 2026-05-06).** Job 5 driver loops MUST gate `run-job5.mjs --finalize` on the `SAVED=true` line from `sa-push-devbrowser.sh` output. Unconditional finalize after a SAVED=false push moves payloads to `done/` despite no SA mutation occurring — leaves SA in pre-edit state while pipeline marks the stem complete. Sample loop: `OUT=$(STEM=$S bash scripts/sa-push-devbrowser.sh); SAVED=$(echo "$OUT" | grep '^SAVED=' | cut -d= -f2); [ "$SAVED" = "true" ] && STEM=$S node scripts/run-job5.mjs --finalize`. Reason: Dashboard batch incident — 5 stems audit-failed, SAVED=false logged correctly, but unconditional finalize archived all 5 to done/ requiring manual revert + queue restoration.
5. Click task-level **Save**. Do NOT wait for a save toast — SA does not emit one (codified 2026-05-05 — Box_plot_4 incident: `saSave()` burned 10s polling for a "saved" toast that never appears, plus a follow-up reload+readback added another ~5s; both unnecessary because the pre-save audit at step 4 already verified state). Past dev-browser flow ran ~9s/stem precisely because it skipped both. Click Save and proceed straight to finalize.
...
- **CLI clicks per-annot X (`.delete-btn`) BEFORE Save, in the same Save event (codified 2026-05-09, Igor override of 2026-05-08 split-flow).** The per-annot X icon in SA's annot panel removes the annot from the in-memory editor state but is REVERSIBLE until Save fires — same risk profile as a skill-checkbox toggle. `scripts/sa-push-devbrowser.sh` integrates this into the standard Job 5 push: after applying edits + pre-save audit, the script clicks `.delete-btn[n-1]` for each `action: delete` annot in DESCENDING n order (so prior clicks don't shift later indices), then verifies every remaining QC rating is `svgicon=approve-action` (👍), then clicks Save. Single Save event commits edits + deletes atomically. Refuses Save if any 👎 remains post-delete. The task-level Delete (the trash icon on the task row, not the per-annot X) IS irreversible and remains Igor-only.
  - **Per-annot `.delete-btn` shows on EVERY annot, not just 👎 (codified 2026-05-08 — V5 batch #4 incident).** Earlier premise — "SA renders one X per thumbs-down annot post-Job-5-save" — was wrong. The X icon is a per-annot remove handle that appears for all annots regardless of rating. Blanket-loop click logic (click `.delete-btn[0]` until none remain) would delete ALL annots, not just the 👎 ones. Mitigation: target by index using payload's `n` field, plus a post-click verification gate that refuses Save unless every remaining QC rating is `approve-action`. PROJECT_ID auto-detected from queue file when present; after Job 5 finalize the queue file is gone, so for V5 stems pass `PROJECT_ID_OVERRIDE=283665` (V6 default is 290044). Reason: V5 batch #4 first delete attempt clicked `.delete-btn[0]` instead of the targeted A5 → would have wiped A1; reload restored state, then targeted-by-index + rating-readback gate completed the 4 deletes safely.
...
- **Date stamps always PST (codified 2026-05-09).** All `M/D:` and `YYYY-MM-DD` date generators in scripts now use `Intl.DateTimeFormat('en-US'/'en-CA', { timeZone: 'America/Los_Angeles' })` so they produce PST dates regardless of `TZ` env or host timezone. Affected files: `scripts/job2-merge.mjs` (mdToday for feedback), `scripts/audit-skill-tags.mjs`, `scripts/backfill-skill-deltas.mjs`, `scripts/run-job1.mjs` (skeleton header date). Reason: pre-fix, the `new Date().getMonth()/getDate()` form returned the host-local date; on a UTC-set host or after midnight UTC, this produced wrong M/D vs Igor's PST workday. Use the helper pattern when adding new date-stamped output.

- **Required-feedback guardrail in saPreSaveAudit (codified 2026-05-09 — Tech_53/Travel_69/Work_efficiency_1 incident).** When `skills_check` or `skills_uncheck` is non-empty, OR when `rating == thumbs-down`, the audit fails if `sa.feedback` is null/empty. Earlier audit only verified "feedback is in textarea IF non-null" — silently allowed null feedback when skills were patched after Job 2 auto-resolve. Result: SA got skill edits applied with no QC explanation. Fix path: when manually patching skills_check/skills_uncheck on a payload (e.g. to clear skill-audit-sanity), ALWAYS update the feedback field too with a `M/D: ...` line explaining the skill change. The audit now refuses Save until both fields are aligned.
