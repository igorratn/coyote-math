...
% 3x3 tiling around the central rhombus
\foreach \i in {-1,0,1}{
  \foreach \j in {-1,0,1}{
    \draw[very thin,gray!40]
      ($(\i,0)+\j*(\cx,\cy)$) rectangle ++(1,0.8660254);
  }
}
...
% Draw Voronoi cells (hexagons around lattice points)
% For a triangular lattice, the Voronoi cells are regular hexagons.
% The vertices are at the midpoints between a lattice point and its six closest neighbors.
...
\foreach \i in {-1,0,1}{
  \foreach \j in {-1,0,1}{
    \draw[line width=0.5pt, gray, dashed, opacity=0.7]
      ($(\i*\bx+\j*\cx, \i*\by+\j*\cy)+(HexV1)$) --
      ($(\i*\bx+\j*\cx, \i*\by+\j*\cy)+(HexV2)$) --
      ($(\i*\bx+\j*\cx, \i*\by+\j*\cy)+(HexV3)$) --
      ($(\i*\bx+\j*\cx, \i*\by+\j*\cy)+(HexV4)$) --
      ($(\i*\bx+\j*\cx, \i*\by+\j*\cy)+(HexV5)$) --
      ($(\i*\bx+\j*\cx, \i*\by+\j*\cy)+(HexV6)$) -- cycle;
  }
}
